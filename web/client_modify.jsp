...
try{ 
        ...
        PreparedStatement pst=con.prepareStatement("select * from clientupload where cloud = 'cloud1'  and owner = '"+user+"' ");
...
<%
	...
	while(rs.next()){
            %><tr>
                ...
                <td><a href="client_modify1.jsp?filename=<%=rs.getString(2)%>&owner=<%=rs.getString("owner")%>&cloud1data=<%=rs.getString("filedata").replaceAll("\\+", "%2B")%>&fileid=<%=rs.getInt(1)%>&fname=<%=rs.getString("fname")%>&fkey=<%=rs.getString("fkey").replaceAll("\\+", "%2B")%>"><font color="#0d637d" size="3">View</font></a> </td>
